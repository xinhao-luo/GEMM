...
def read_data(data_path, log_files):
    ...
    for log_file in log_files:
        ...
        # for row in data:
        #     print(row[3])
        data_throughput.append([float(row[3]) for row in data])
        data_time.append([float(row[4].split('ms')[0]) for row in data])
    # print(len(data_throughput))
    return data_throughput, data_time


def get_dims(log_files):
    dims = [128, 256, 512, 768, 1024, 2048, 3072, 4096, 5120, 6144, 7168]
...
def analyze_data(data_path):
    ...
    draw_line_chart(methods, dims, data_throughput, data_path +
                    'gemm_throughput.png', 'Throughput / TFLOPS', 'GEMM Throughput')
    draw_line_chart_time(methods, dims, data_time, data_path +
                    'gemm_time.png', 'Time / ms', 'GEMM Time')


usage = "python3 performance.py -p/--path exp_new/"
parser = optparse.OptionParser(usage)
parser.add_option('-p', '--path', dest='path',
                    type='string', help='data path', default='exp_new/')
