...
{
    int global_row = blockIdx.y * wmmaM;
    int global_col = blockIdx.x * wmmaN;
    ...
    // Step 1: Load A and B to shared memory
    __shared__ float* shared_A[wmmaM * wmmaK];
    __shared__ float* shared_B[wmmaK * wmmaN];

    int shared_a_offset = threadIdx.x;
    int shared_b_offset = threadIdx.x;
    // printf("x: %d, y: %d, \n", shared_a_offset, shared_b_offset);
    // Step 2: Load A and B from global memory to shared memory
    int load_id_a = global_row * K;
    int load_id_b = global_col;
    // printf("x: %d, y: %d, \n", load_id_a, load_id_b);
    if(shared_a_offset < wmmaM * wmmaK && load_id_a < M * K)
        shared_A[shared_a_offset] = A + load_id_a;
    if(shared_b_offset < wmmaK * wmmaN && load_id_b < K * N)
        shared_B[shared_b_offset] = B + load_id_b;
    ...
    for (int i = 0; i < K; i += wmmaK) {
        ...
        wmma::load_matrix_sync(b_frag, shared_B[shared_b_offset], N);
        ...
        shared_a_offset = (shared_a_offset + threadIdx.x) % (wmmaM * wmmaK);
        shared_b_offset = (shared_b_offset + threadIdx.x) % (wmmaK * wmmaN);
    ...
    int output_id = (global_row * N) + global_col;
    if(output_id < M * N)
