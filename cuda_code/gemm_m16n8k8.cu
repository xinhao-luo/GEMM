...
)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        C_frag[0][0][i] = __float2half(0.0f);
    ...
    // printf("bid.x: %d, bid.y: %d, tid: %d, a_smem: %f, \n", blockIdx.x, blockIdx.y, threadIdx.x, __half2float(*(a_smem + shared_A_consume_start)));
            ...
            for (int _n = 0; _n < WARP_TILE_N; _n += MMA_TILE_N) {
                ...
                for (int _k = 0; _k < WARP_TILE_K; _k += MMA_TILE_K) {
                    ...
                    asm volatile (
                        ...
                        : "=r"(*(uint32_t*)(&C_frag[_m / MMA_TILE_M][_n / MMA_TILE_N][0])), 
                          "=r"(*(uint32_t*)(&C_frag[_m / MMA_TILE_M][_n / MMA_TILE_N][2]))
                        : "r"(*(uint32_t*)(&A_frag[0])), 
                          "r"(*(uint32_t*)(&A_frag[2])), 
                          "r"(*(uint32_t*)(&B_frag[0])), 
                          "r"(*(uint32_t*)(&C_frag[_m / MMA_TILE_M][_n / MMA_TILE_N][0])), 
                    ...
                    asm volatile (
                        ...
                        : "=r"(*(uint32_t*)(&B_frag[0]))
                        : "r"(convert_shmem_ptr_to_uint32(b_smem + shared_B_consume_start + ((_k + MMA_TILE_K) % WARP_TILE_K) * (BLOCK_TILE_N + SHARED_MEM_OFFSET)))
                    );
                    // printf("end, \n");
    ...
    // printf("wb_idx: %d\n", wb_idx);
    *(uint32_t*)(&D[wb_idx]) = *(uint32_t*)(&C_frag[0][0][0]);
    *(uint32_t*)(&D[wb_idx + 8 * n]) = *(uint32_t*)(&C_frag[0][0][2]);
}
...
int main(int argc, char** argv) {
    ...
    // for (int i = 0; i < M * N; i++) { 
    //     if(!__half2float(h_D[i]))
    //         printf("%d, %8.3f\n", i, __half2float(h_D[i]));
    // }
    // for (int i = 0; i < 10; i++) { 
    //     for (int j = 0; j < 10; j++) {
    //         printf("%8.3f", __half2float(h_D[i * N + j]));
    //     }
    // }
