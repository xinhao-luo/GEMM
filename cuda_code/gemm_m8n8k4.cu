...
)
{
    ...
    half C_frag[8];
    for (int i = 0; i < 8; i++) {
        A_frag[i / 2] = __float2half(0.0f);
        C_frag[i] = __float2half(0.0f);
    ...
    if(lane_id < 16) {
        ...
        load_shared_B_lane_id = ((lane_id - 16) % 4);
    ...
    *(uint64_t*)(&B_frag[0]) = *(uint64_t*)(b_smem + shared_B_consume_start);
        ...
        for (int _m = 0; _m < WARP_TILE_M; _m += MMA_TILE_M) {
            for (int _n = 0; _n < WARP_TILE_N; _n += MMA_TILE_N) {
                ...
                for (int _k = 0; _k < WARP_TILE_K; _k += MMA_TILE_K) {
                    ...
                    // printf("bid.x: %d, bid.y: %d, tid: %d, c_reg: %f, \n", blockIdx.x, blockIdx.y, threadIdx.x, __half2float(C_frag[0][0][0]));
                    // printf("mma over, \n");
    ...
    if(lane_id < 16) {
        if(lane_id < 8) 
            ...
            wb_idx += ((lane_id / 12) * 8 + (lane_id % 4)) * N;
    } else {
        if(lane_id < 24) 
            wb_idx += (4 + (lane_id / 20) * 8 + (lane_id % 4)) * N;
        else 
            wb_idx += (4 + (lane_id / 28) * 8 + (lane_id % 4)) * N;
    }
...
int main(int argc, char** argv) {
    ...
    // for (int i = 0; i < 10; i++) { 
    //     for (int j = 0; j < 10; j++) {
    //         printf("%8.3f", __half2float(res[i * N + j]));
    //     }
    // }
